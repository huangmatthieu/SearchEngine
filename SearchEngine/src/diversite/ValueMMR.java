...
public class ValueMMR implements Value {
	private Index idx;
	private double alpha;
	public ValueMMR(Index idx,double alpha){
		this.idx = idx;
		this.alpha=alpha;
	}
	...
	public Integer value(HashMap<String, Integer> query,ArrayList<Integer> documents) {
		...
		resultat.put(pivot,simDocQuery.get(pivot));
		//On va jusqu'a i qui vaut 30
		for(int i = 0 ; i < 30 ; i++){
			...
			for(int j = 0 ; j < numDocs.size(); j++){
				...
				if(!maxSim.containsKey(numDocs.get(j))){
					...
					maxSim.get(numDocs.get(j)).add(res);
				}
